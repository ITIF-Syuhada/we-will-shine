...
<svelte:head>
	...
	<meta
		name="description"
		content="Login ke We Will Shine - AI Learning Assistant untuk produktivitas belajar optimal"
	/>
</svelte:head>
...
<div class="flex min-h-screen">
		...
		<div class="mx-auto w-full max-w-md">
			...
			<div class="mb-4 flex gap-2 rounded-lg bg-gray-100 p-1">
				<button
					...
					class="flex-1 rounded-md px-4 py-2 text-sm font-semibold transition-all {activeTab ===
					'email'
			...
			<!-- Sign In Form Card -->
			<div
				class="flex min-h-[320px] flex-col justify-center rounded-2xl border-2 border-purple-200 bg-white p-6 shadow-xl"
			>
				<!-- Success Message -->
				{#if success}
					<div
						class="mb-4 rounded-lg border-2 border-green-200 bg-green-50 p-3 text-center text-sm text-green-600"
					>
						✅ {success}
					</div>
				{/if}
					...
					<div class="space-y-3">
						...
						<div>
							<div class="mb-2 flex items-center justify-between">
								...
								<button class="text-xs text-purple-600 hover:underline"> Lupa password? </button>
							</div>
							<input
								id="password"
								type="password"
				...
				{#if activeTab === 'code'}
					<div class="space-y-3">
						<div class="rounded-lg bg-gradient-to-br from-indigo-50 to-purple-50 p-5 text-center">
							...
							<p class="mb-3 text-xs text-gray-600">Punya kode unlock dari sekolah?</p>
							<button
	...
	<div
		class="relative hidden lg:flex lg:flex-1 lg:flex-col lg:items-center lg:justify-center lg:overflow-hidden lg:bg-gradient-to-br lg:from-red-500 lg:via-orange-400 lg:to-yellow-400 lg:p-8 lg:text-white"
	>
		<!-- Background Pattern -->
		<div class="absolute inset-0 opacity-10">
			<div
				class="h-full w-full"
				style="background-image: radial-gradient(circle at 30px 30px, rgba(255, 255, 255, 0.3) 2px, transparent 2px); background-size: 60px 60px;"
			></div>
		</div>

		<!-- Logo & Brand - Top Right Corner -->
		<div class="absolute top-8 right-8 z-20 text-right">
			<a href="{base}/" class="inline-block transition-transform hover:scale-105">
				<div class="mb-2 flex items-center justify-end gap-2">
					<div
						class="flex h-12 w-12 items-center justify-center rounded-full bg-white/20 text-2xl shadow-lg backdrop-blur-sm"
					>
						🌟
					</div>
				</div>
		...
		<div class="relative z-10 max-w-xl">
			...
			<!-- Quote Card -->
			<div class="rounded-3xl bg-white/10 p-6 shadow-2xl backdrop-blur-md">
				<!-- Quote Icon -->
				<div class="mb-4 text-5xl">📜</div>

				<!-- Quote Text -->
				<blockquote class="mb-4 text-xl leading-relaxed font-bold">
					"{currentQuote.quote}"
				</blockquote>
		...
		<div class="absolute bottom-8 left-1/2 -translate-x-1/2 text-center text-white">
			...
			<p class="text-base font-semibold">Membangun Generasi Emas 2045</p>
